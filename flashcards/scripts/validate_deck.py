...
def load_deck_data():
    ...
    with open(DECK_DATA_FILE, encoding="utf-8") as f:
        data = json.load(f)
...
def parse_md_source():
    ...
    with open(MD_SOURCE_FILE, encoding="utf-8") as f:
        content = f.read()
    ...
    for _, line in enumerate(lines[data_start:], start=data_start):
        line = line.strip()
...
def validate_gender_article(card):
    ...
    # Check consistency
    expected_gender = {"der": "m", "die": "f", "das": "n"}

    if article in expected_gender and gender != expected_gender[article]:
...
def generate_report(deck_data, md_ids, md_cards, orphaned, missing, validation_issues):
    ...
    if orphaned:
        ...
        lines.append(
            "They were likely removed from the source for a reason (excluded categories, errors, etc.)"
        )
        lines.append("")
        ...
        for i, card in enumerate(orphaned, 1):
            ...
            if fields.get("Example_DE"):
                lines.append(f"- **Example (DE):** {fields['Example_DE'][:100]}...")
            if fields.get("Example_RU"):
    ...
    # Duplicate IDs
    if validation_issues.get("duplicate_ids"):
    ...
    if validation_issues.get("gender_mismatches"):
    ...
    # Empty fields
    if validation_issues.get("empty_fields"):
    ...
    # Cloze syntax
    if validation_issues.get("cloze_errors"):
...
def main():
    ...
    for card in deck_data["cards"]:
        ...
        if gender_issue:
            validation_issues["gender_mismatches"].append({"guid": guid, "message": gender_issue})

        # Empty fields check
        empty_issue = validate_empty_fields(card)
        if empty_issue:
            validation_issues["empty_fields"].append({"guid": guid, "issues": empty_issue})

        # Cloze syntax check
        cloze_issue = validate_cloze_syntax(card)
        if cloze_issue:
            validation_issues["cloze_errors"].append({"guid": guid, "message": cloze_issue})
